...
from .pos_embeddings import get_gaussian_pos, get_sinusoidal_2d, get_raw_coordinate
...
class TNT(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, outer_dim=768, inner_dim=48,
        ...
        if self.pos_emb == 'absolute_learnable':
            ...
            self.pos_weight = nn.Linear(num_patches, outer_dim)
            self.cls_pos = nn.Parameter(torch.zeros(1, 1, outer_dim))
            trunc_normal_(self.cls_pos, std=.02)
        elif self.pos_emb == 'sinusoid_2d':
            print('using sinusoid_2d')
            height = width = int(math.sqrt(num_patches))
            self.cls_pos = nn.Parameter(torch.zeros(1, 1, outer_dim))
            self.sin_pos = nn.Parameter(get_sinusoidal_2d(outer_dim, height, width, torch.Tensor([10000])), requires_grad=False)
            trunc_normal_(self.cls_pos, std=.02)
            self.outer_pos = nn.Parameter(torch.cat((self.cls_pos, self.sin_pos), dim=1))
            ...
            self.sin_model = LearnableSinusoidPositionalEncoding(2, outer_dim // 2, outer_dim,
                                                                 4 * outer_dim)
    ...
    def forward_features(self, x):
        ...
        if self.pos_emb == 'absolute_learnable' or self.pos_emb == 'sinusoid_2d':
            outer_tokens = outer_tokens + self.outer_pos
        elif self.pos_emb == 'gaussian_2d':
            outer_pos = torch.cat((self.cls_pos, self.pos_weight(self.pos).unsqueeze(0)), dim=1)
            outer_tokens = outer_tokens + outer_pos
        elif self.pos_emb == 'sinusoid_learnable_new':
            outer_pos = self.sin_model(self.pos)
            outer_tokens = outer_tokens + outer_pos
        ...
        outer_tokens = torch.mean(outer_tokens, dim=1)
        return outer_tokens
