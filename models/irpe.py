...
class iRPE(nn.Module):
    ...
    def _get_rp_bucket(self, x, height=None, width=None):
        ...
        assert num_buckets == self.num_buckets - 1
    ...
    def forward_rpe_transpose(self, x, rp_bucket):
        ...
        if self.mode == 'bias':
            ...
            """
            # Currently:
            # h b*l d, h d s -> h b*l s -> h b l s
            # h b*l d, b h d s -> b h b*l s -> b*h b l s
            # Should be:
            # b h l d, b h d s -> b h l s
            if self.lookup_table_weight.dim() == 4:
                # We have overridden the lookup_table_weight to have separate
                # values for each sample
                lookup_table = torch.einsum("b h l d, b h d s -> b h l s", x, self.lookup_table_weight)
            else:
