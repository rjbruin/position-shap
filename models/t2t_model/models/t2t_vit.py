...
from .pos_embeddings import get_sinusoidal_2d, get_gaussian_pos, get_raw_coordinate
...
class T2T_ViT(nn.Module):
    def __init__(self, img_size=224, tokens_type='performer', in_chans=3, num_classes=1000, embed_dim=768, depth=12,
                 ...
                 drop_path_rate=0., norm_layer=nn.LayerNorm, token_dim=64, pos_emb='sinusoid_1d'):
        super().__init__()
        ...
        if self.pos_emb == 'sinusoid_1d':
            print('using sinusoid 1d')
            self.pos_embed = nn.Parameter(data=get_sinusoid_encoding(n_position=num_patches + 1, d_hid=embed_dim), requires_grad=False)
        elif self.pos_emb == 'absolute_learnable':
            print('using abs learnable')
            ...
            self.sin_pos = nn.Parameter(get_sinusoidal_2d(embed_dim, height, width, torch.Tensor([10000])), requires_grad=False)
            trunc_normal_(self.cls_pos, std=.02)
            self.pos_embed = nn.Parameter(torch.cat((self.cls_pos, self.sin_pos), dim=1))
            ...
            print('using learnable sinusoid in t2t')
    ...
    def forward_features(self, x):
        ...
        if self.pos_emb == 'gaussian_2d':
            gaussian_pos = self.pos_weight(self.pos).unsqueeze(0)
            self.pos_embed = torch.cat((self.cls_pos, gaussian_pos), dim=1)
            x = x + self.pos_embed
        else:
            x = x + self.pos_embed
        x = self.pos_drop(x)
