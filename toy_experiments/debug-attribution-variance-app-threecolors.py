...
pos_colors = []
train_images, train_labels, test_images, test_labels, n_classes = datasets.dataset_appearance_absolute_position(size=[8,8], colors=colors, pos_colors=pos_colors, split_quadrants=True)
print(f"{n_classes} classes")

size = 8
patch_size = 2
dataset_settings = Setting(size=size, patch_size=patch_size, n_classes=n_classes, colors=colors, pos_colors=pos_colors)
...
# Run 18: wide search on appearance-only toy, run on cluster
run_name = 'run20-sens+shap-appthreecolors'
save_plots = True
# len(settings) = 30
settings = [
    Setting(lr=4e-3, weight_decay=1e-3, d=4,  n_heads=2, pos_emb_factor=0.1,  pos_add='concat_equald', seeds=range(1), **base_settings),
